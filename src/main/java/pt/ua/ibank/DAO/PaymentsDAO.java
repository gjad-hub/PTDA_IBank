...
public class PaymentsDAO {
    ...
    public static int payService(double valor, int clienteRealiza, int cliente_criador, int entidade, int referencia) {
        PreparedStatement stmt = null;
        try {

            stmt = conn.prepareStatement("UPDATE pagamento_servicos_compras SET pago = ?, cliente = ?, cliente_cria = ? WHERE entidade = ? and referencia = ?");
        ...
        try {
            ...
            while (rs.next()) {
                service = new PagamentoServicosCompras(rs.getInt("referencia"), rs.getInt("entidade"),
                        rs.getDouble("valor"), rs.getBoolean("pago"), rs.getInt("cliente"), rs.getInt("cliente_cria"), rs.getBoolean("cancelada"));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        } finally {
    ...
    public static int createPayment(int cliente, int ent, int ref, double valor) {
        ...
        try {
            ...
            stmt = conn.prepareStatement("INSERT INTO pagamento_servicos_compras (entidade, referencia, valor, pago, cliente_cria, cancelada)"
                    + "VALUES (?, ?, ?, ?, ?, ?)");
            stmt.setInt(1, ent);
            stmt.setInt(2, ref);
    ...
    public static ArrayList<PagamentoServicosCompras> getAllServicos(int num_cliente) {
        PreparedStatement stmt = null;
        ResultSet rs = null;
