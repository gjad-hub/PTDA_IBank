...
public class CardsDAO {
        ...
        try {
            ...
            while (rs.next()) {
                Cartao tr = new Cartao(rs.getString("num_cartao"),
                        rs.getDate("data_validade"), rs.getString("estado"), rs.getBoolean("credito"));
                lcartao.add(tr);
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public static int cancelCard(String card_number) {
        ...
        try {
            stmt = conn.prepareStatement("UPDATE cartao SET estado = \"cancelado\" WHERE num_cartao LIKE ? ");
    ...
    public static int makeDefault(String card_number, int num_cliente) {
        ...
        try {
            stmt = conn.prepareStatement("UPDATE cliente SET cartao_default = ? WHERE num_cliente = ?");
            stmt.setString(1, card_number);
    ...
    public static int createCard(int num_cliente) {
        ...
        try {
            ...
            stmt = conn.prepareStatement("INSERT INTO cartao (num_cartao, data_validade, estado, cliente) "
